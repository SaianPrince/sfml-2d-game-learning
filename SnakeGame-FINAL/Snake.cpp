...
{
	if (s_cells[0].m_getDirection() == cellDirection::Right && newDirection == cellDirection::Left) return;
	if (s_cells[0].m_getDirection() == cellDirection::Left && newDirection == cellDirection::Right) return;
	if (s_cells[0].m_getDirection() == cellDirection::Up && newDirection == cellDirection::Down)	 return;
	if (s_cells[0].m_getDirection() == cellDirection::Down && newDirection == cellDirection::Up)	 return;
...
{
	for (int i=1; i < s_cells.size() - 1; i++)
	{
		if (s_cells[i].m_getLocation() == s_cells[0].m_getLocation())
			return true;
	}
	return false;
}
